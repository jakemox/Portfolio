<!DOCTYPE html>
<!--[if gt IE 8]><!-->
<html class="no-js">
<!--<![endif]-->

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>Portfolio</title>
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="style.css">
    <link href="https://fonts.googleapis.com/css?family=Poppins:200,400,500" rel="stylesheet">
    <link rel="icon" href="Favicon-3.png" sizes="16x16 32x32" type="image/png">
</head>



<body>
    <div class="thanks">
        Thanks for your email!
    </div>
    <header>
        <div class="container">
            <div class="main">
                <div class="abc">
                    <a href="index.html">
                        <img id="logo" class="logo" src="images/Favicon 4.svg" alt="Logo">
                    </a>   
                    <nav>
                        <div id="current-item" class="burger">&#x2630;</div>
                        <div id="navigation" class="nav1 dropdown">
                            <a href="#top">Home</a>
                            <a href="#skills">Skills</a>
                            <a href="#folio">Portfolio</a>
                            <a href="#info">About Me</a>
                            <a href="#contact">Contact</a>
                        </div>
                    </nav>
                </div>
                <div class="hero">
                    <h1>Jake Moxon</h1>
                    <h2>Web Developer</h2>
                </div>
            </div>
        </div>
    </header>
    <section class="general">
        <div class="intro">
            <p>
                Let's work together to make <b>your product stand out</b> . I create beautiful and functional websites. I work with my clients step by step <b>from the idea stage until the fully functioning web application</b>.
            </p>
        </div>
    </section>
    <section class="discover">
        <div class="container">
            <div class="dds">
                <div class="ddd">
                    <h3>Discover</h3>
                    <p>Tell me about your project and we brainstorm together the features to create a plan for your product to launch.</p>
                </div>
                <div class="ddd">
                    <h3>Design</h3>
                    <p>We design the wireframe and agree together how to create the best experience for your users to present your brand.</p>
                </div>
                <div class="ddd">
                    <h3>Deploy</h3>
                    <p>We protoype and develop your product. We work together intensily to incorporate your feedback.</p>
                </div>
                <div class="ddd">
                    <h3>Be Ready</h3>
                    <p>We deploy your product and now your website is ready to get the traction. Get out there and grow your audience.</p>
                </div>
            </div>
        </div>
    </section>
    <section class="myskills">
        <div class="container">
            <h2 name="skills">My Skills</h2>
            <div class="progress">
                <div class="skill1">HTML</div>
                <div class="bar">
                    <div class="knob" style="width: 90%"></div>
                </div>
                <div class="perc">90%</div>
            </div>
            <div class="progress">
                <div class="skill1">CSS</div>
                <div class="bar">
                    <div class="knob" style="width: 85%"></div>
                </div>
                <div class="perc">85%</div>
            </div>
            <div class="progress">
                <div class="skill1">JavaScript</div>
                <div class="bar">
                    <div class="knob" style="width: 75%"></div>
                </div>
                <div class="perc">75%</div>
            </div>
            <div class="progress">
                <div class="skill1">React</div>
                <div class="bar">
                    <div class="knob" style="width: 75%"></div>
                </div>
                <div class="perc">75%</div>
            </div>
            <div class="progress">
                <div class="skill1">jQuery</div>
                <div class="bar">
                    <div class="knob" style="width: 80%"></div>
                </div>
                <div class="perc">80%</div>
            </div>
            <div class="progress">
                <div class="skill1">PHP</div>
                <div class="bar">
                    <div class="knob" style="width: 20%"></div>
                </div>
                <div class="perc">20%</div>
            </div>
            <div class="progress">
                <div class="skill1">Laravel</div>
                <div class="bar">
                    <div class="knob" style="width: 10%"></div>
                </div>
                <div class="perc">10%</div>
            </div>
            <div class="progress">
                <div class="skill1">UX</div>
                <div class="bar">
                    <div class="knob" style="width: 30%"></div>
                </div>
                <div class="perc">30%</div>
            </div>
        </div>
    </section>
    <section class="portfolio">
        <div class="container">
            <h2 name="folio">My Portfolio</h2>
            <div class="projects">
                <div class="ppp">
                    <div id="hoverarea" class="img photo1" style="background-image: url('/images/Bow-River.jpg')">
                        <a class="project1" href="">
                            <div id="hovereffect" class="place hover1"><span>Banff</span></div>
                        </a>
                    </div>
                    <div class="img photo2" style="background-image: url('/images/Tallinn-3.jpg')">
                        <a class="project1" href="">
                            <div class="place hover2"><span>Tallinn</span></div>
                        </a>
                    </div>
                    <div class="img photo3" style="background-image: url('/images/Tatra-Snow.jpg')">
                        <a class="project1" href="">
                            <div class="place hover3"><span>Zakopane</span></div>
                        </a>
                    </div>
                    <div class="img photo4" style="background-image: url('/images/Pigeon.jpg')">
                        <a class="project1" href="">
                            <div class="place hover4"><span>Rome</span></div>
                        </a>
                    </div>
                    <div class="img photo5" style="background-image: url('/images/Maya-Bay.jpg')">
                        <a class="project1" href="">
                            <div class="place hover5"><span>Thailand</span></div>
                        </a>
                    </div>
                    <div class="img photo6" style="background-image: url('/images/Lake-Bled.jpg')">
                        <a class="project1" href="">
                            <div class="place hover6"><span>Bled</span></div>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="aboutme">
        <div class="container">
            <h2 name="info">About Me</h2>
            <div class="aaa">
                <img class="me" src="images/Jake.jpg" alt="Jake Moxon">
                <div class="text">
                    <p>
                        <b>My name is Jake Moxon.</b> I am a full-stack web developer and enjoy learning new trends. <b>I am developing user interfaces in JavaScript and React and backend of my projects in PHP and Laravel.</b>
                        <br>
                        <br>
                        I am open to learning new technologies and enjoy working with people who are as passionate about all web-related as me.
                        <br>
                        <br>
                        I have worked for clients all across the world but currently I am based in Prague, Czech Republic. 
                        <br>
                        <br>
                        I like cats, gaming, and snowboarding. If I do not develop, you can also meet me trekking in the mountains.
                    </p>
                </div>
            </div>
        </div>
    </section>
    <section class="findme">
        <div class="container">
            <h2>Find me here</h2>
            <div class="social">
                <a href="https://www.instagram.com/jakemox99/" target="_blank"><img class="icons" src="images/instagram.svg" alt="instagram"></a>
                <a href="https://github.com/jakemox" target="_blank"><img class="icons" src="images/github.svg" alt="github"></a>
                <a href="https://www.linkedin.com/in/jake-moxon-74a541134/" target="_blank"><img class="icons" src="images/linkedin.svg" alt="linkedIn"></a>
            </div>
        </div>
    </section>
    <section class="contactme">
        <div class="container">
            <h2 name="contact">Contact Me</h2>
            <!-- <div class="form"> -->
                <form class="form" id="contact" action="contact.php" method="post">
                    <div class="field1">
                        <img src="images/profile-01.svg" alt="" width="40px" height="40px">
                        <input name="name" placeholder="Your Name*" type="text" tabindex="1" required>
                    </div>
                    <div class="field1">
                        <img src="images/email.svg" alt="" width="40px" height="40px">
                        <input name="email" placeholder="Your Email*" type="email" tabindex="3" required>
                    </div>
                    <div class="field2"><p>Your message*</p><textarea name="message" placeholder="Your message*" tabindex="5" required></textarea></div>
                    <button name="submit" type="submit" id="contact-submit" data-submit="...Sending">Send Message</button>
                </form>
            <!-- </div> -->
        </div>
    </section>
    <section class="nice">
        <div class="container">
            <h2>Nice to meet you!</h2>
            <p>Copyright &copy; Jake Moxon</p>
        </div>
    </section>
    <script src="main.js"></script>
</body>

</html>